      ...
      if ((bot.status === 'WORKING' || bot.status === 'ASSIGNED' || bot.status === 'EN_ROUTE') && bot.current_task_id) {
        ...
        if (task) {
          ...
          const isNotInChargingFlow =
            (bot.status as string) !== 'CHARGING' &&
            (bot.status as string) !== 'MOVING_TO_CHARGER' &&
            (bot.status as string) !== 'WAITING_FOR_CHARGER' &&
            (bot.status as string) !== 'RETURNING';

          if (isPredictiveHighRisk && !bot.predictive_risk_handled && !bot.predictive_risk_ack_pending && isNotInChargingFlow) {
    ...
    for (const bot of state.uavs) {
      ...
      if (isLow && !bot.low_battery_handled && !bot.low_battery_ack_pending && isNotInChargingFlow) {
        ...
        state.pending_acknowledgement = {
          ...
          timestamp: now,
          reason: `${bot.id} reached emergency battery threshold (${bot.battery.toFixed(1)}%). Requires charging station dispatch.`
        };
